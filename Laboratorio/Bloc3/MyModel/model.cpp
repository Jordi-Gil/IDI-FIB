...
static void ompleVBOs(vector<Face> &_faces,
	              ...
	              vector<Normal> const &_normals,
								vector<Textura> const &_texturas,
		      float *&_VBO_vert, float *&_VBO_norm, float *&_VBO_text,
		      float *&_VBO_mata, float *&_VBO_matd, float *&_VBO_matsp, float *&_VBO_matsh);

static bool fvtn = false;
static bool fvt = false;
static bool texcoord = true;
static string modelPath("");

// ======== Constructors and Destructors =======
Model::Model() : _vertices(0), _normals(0), _texturas(0),_faces(0) {
  _VBO_vertices = _VBO_normals = _VBO_texturas = _VBO_matamb = _VBO_matdiff = _VBO_matspec = _VBO_matshin = NULL;
}
...
void Model::load(std::string filename) {
  if (! _vertices.empty()) {
    ...
    _normals.erase(_normals.begin(), _normals.end());
		_texturas.erase(_texturas.begin(), _texturas.end());
  ...
  while (getline(input, line)) {
	...
	for (int i = 0; i < 2; ++i) { ss >> coord; _texturas.push_back(coord);}
	break;
      default:
