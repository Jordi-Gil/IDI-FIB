...
{
    Modelminim.x = Modelmaxim.x = homer.vertices()[0];
    Modelminim.y = Modelmaxim.y = homer.vertices()[1];
    Modelminim.z = Modelmaxim.z = homer.vertices()[2];
    
    for(unsigned int i = 3; i < homer.vertices().size(); i+=3){
        //Comprovamos la componente 'x'
        Modelminim.x = std::min(Modelminim.x,(float) homer.vertices()[i]);
        Modelmaxim.x = std::max(Modelmaxim.x,(float) homer.vertices()[i]);
        
        //Comprovamos la componente 'y'
        Modelminim.y = std::min(Modelminim.y,(float) homer.vertices()[i+1]);
        Modelmaxim.y = std::max(Modelmaxim.y,(float) homer.vertices()[i+1]);
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_M: {
        if(gir) {
            player.setMedia(QUrl::fromLocalFile("/dades/jordi.gil.gonzalez/IDI/Bloc2/Sesion5/Brusko Problemz - Industrial Hardcore.mp3"));
            player.play();
        }
        else player.stop();    
    }
